...
//! `[shortcut] translate_selection`，切换键在 `[shortcut] switch_mode` 选 `ctrl+space` 时才登记
//! （[`settings`](crate::com::settings)，配置改了就地重登记）。
...
/// 读配置里的翻译组合；解析失败时配置整体退回缺省，这里拿到的也是缺省组合。
pub(crate) fn load_combo() -> KeyCombo {
    settings::load().shortcut.translate_selection
}
